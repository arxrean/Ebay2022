...
class BertBaseline(pl.LightningModule):
	...
	def training_step(self, batch, batch_idx):
		input_ids, attention_mask, labels, is_add_ons, label_distris = batch
		if self.opt.addon_w != 1:
			_, logits = self.model(input_ids=input_ids, attention_mask=attention_mask, labels=labels, return_dict=False)
			losses = self.ce_loss(logits.reshape(logits.size(0)*logits.size(1), -1), labels.reshape(-1)).reshape(self.opt.batch_size, -1)
			is_add_ons = is_add_ons * self.opt.addon_w
			is_add_ons[is_add_ons==0] = 1
			loss = (losses * is_add_ons.unsqueeze(1)).mean()
		elif self.opt.tag_distri:
			_, logits = self.model(input_ids=input_ids, attention_mask=attention_mask, labels=labels, return_dict=False)
			losses = self.ce_loss(logits.reshape(logits.size(0)*logits.size(1), -1), labels.reshape(-1)).reshape(self.opt.batch_size, -1)
			loss = (losses * label_distris).mean()
		else:
			loss, logits = self.model(input_ids=input_ids, attention_mask=attention_mask, labels=labels, return_dict=False)
	...
	def validation_step(self, batch, batch_idx):
		input_ids, attention_mask, labels, is_add_ons, _ = batch
